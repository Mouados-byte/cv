...
def optimize_html_for_ats(html_content):
   ...
   for img in soup.find_all('img'):
       ...
       if src and not src.startswith('data:'):
           ...
           base64_data = convert_image_to_base64(src)
           if base64_data:
               # Determine image type
               ext = os.path.splitext(src)[1].lower()
               if ext == '.png':
                   img['src'] = f"data:image/png;base64,{base64_data}"
               elif ext in ['.jpg', '.jpeg']:
                   img['src'] = f"data:image/jpeg;base64,{base64_data}"
...
def convert_html_to_pdf(html_file, output_pdf=None, optimize_for_ats=True):
   ...
   try:
       ...
       if os.path.isfile(html_file):
           ...
           os.chdir(html_dir)  # Change to HTML file directory
       ...
       html = HTML(string=html_content, base_url=html_dir)
       ...
       # Add PDF metadata
       metadata = {
           'Title': 'Mouad Imam - Software Developer Resume',
           'Author': 'Mouad Imam',
...
def main():
   ...
   try:
       convert_html_to_pdf(
           args.html_file, 
           args.output, 
           optimize_for_ats=not args.no_optimize
       )
   except Exception as e:
       print(f"Error: {e}")
       return 1
